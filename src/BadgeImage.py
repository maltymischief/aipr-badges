...
class BadgeImage(object):
    ...
    def drawCenteredText(self, pos, text, font, color):
        self.drawAlignedText(pos, text, font, color, lambda x,w:x-w/2, lambda y,h:y-h/2)
    ...
    def drawPerson(self, name):
        ...
        if name.find(" ") >= 0:
            ...
            firstname, rest = (name, "")
        if rest != "":
            personFont = ImageFont.truetype(FONT_NAME, self.getFitSize(FONT_MAXSIZE_NAME, firstname)*300//72)
            self.drawCenteredText(line1pos, firstname, personFont, FONT_COLOR_NAME)
            personFont = ImageFont.truetype(FONT_NAME, self.getFitSize(FONT_MAXSIZE_NAME, rest)*300//72)
            self.drawCenteredText(line2pos, rest, personFont, FONT_COLOR_NAME)
        else:
            personFont = ImageFont.truetype(FONT_NAME, self.getFitSize(FONT_MAXSIZE_NAME, name)*300//72)
            self.drawCenteredText(linepos, name, personFont, FONT_COLOR_NAME)

    def drawCompany(self, name):
        pos = (self.img.size[0]/2, COMPANY_Y)
        font = ImageFont.truetype(FONT_COMPANY, self.getFitSize(FONT_MAXSIZE_COMPANY, name)*300//72)
        self.drawCenteredText(pos, name, font, FONT_COLOR_COMPANY)

    def drawTheme(self, name):
        pos = (THEME_X, THEME_Y)
        font = ImageFont.truetype(FONT_THEME, self.getFitSize(FONT_MAXSIZE_THEME, name)*300//72)
